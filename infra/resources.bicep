...
var webAppSettingsArray = filter(array(webDefinition.settings), i => i.name != '')
var webSecrets = map(filter(webAppSettingsArray, i => i.?secret != null), i => {
  name: i.name
  value: i.value
  secretRef: i.?secretRef ?? take(replace(replace(toLower(i.name), '_', '-'), '.', '-'), 32)
})
var webEnv = map(filter(webAppSettingsArray, i => i.?secret == null), i => {
  name: i.name
  value: i.value
})
